...
class DDPG(object):
    def __init__(self, actor, critic, teacher, memory, memory_D0, memory_D1, observation_shape, action_shape, param_noise=None, action_noise=None,
        ...
        critic_l2_reg=0., actor_lr=1e-4, critic_lr=1e-3, teacher_lr=1e-4, clip_norm=None, reward_scale=1.):
        # Inputs.
    ...
    def merge_memory(self):
        for idx in range(self.memory_D0.nb_entries):
            (obs0, action, reward, obs1, terminal1) = self.memory_D0.getitem(idx)
            self.memory.append(obs0, action, reward, obs1, terminal1)
        self.memory_D0.reset()
        for idx in range(self.memory_D1.nb_entries):
            (obs0, action, reward, obs1, terminal1) = self.memory_D1.getitem(idx)
            self.memory.append(obs0, action, reward, obs1, terminal1)
        self.memory_D1.reset()
        
    def store_transition(self, obs0, action, reward, obs1, terminal1, which_memory=0):
        reward *= self.reward_scale
        if which_memory == 0:
            self.memory.append(obs0, action, reward, obs1, terminal1)
        else if which_memory == 1:
            self.memory_D0.append(obs0, action, reward, obs1, terminal1)
        else if which_memory == 2:
            self.memory_D1.append(obs0, action, reward, obs1, terminal1)
        else :
            raise Exception, 'which_memory = 0,1,2 but here:{}'.format(which_memory)
        if self.normalize_observations:
